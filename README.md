...
- USB 大文件下载改为 1 MiB 分块读取，可在块间立即响应取消，不再等待整段视频传输结束
- 优化 USB 首批相册读取：先返回首屏，再按需补充；设置页可分别配置首次读取与后续加载张数
- 修复 `.xmp` 导入兼容性，包括 Android XML 解析、Adobe RGB Table 解码与压缩数据读取
- 优化 LUT 导入体验：增加导入中提示、统一底部反馈样式，并只显示导入文件名
- 照片页点击缩略图先查看原图，完整原图加载后才展示 LUT 调色面板
- 错误诊断卡片支持手动关闭，关闭提示不会重连或中断当前相机连接
- 修正 USB 缩略图方向识别，并补充分块传输与 XMP 解析自动化测试
